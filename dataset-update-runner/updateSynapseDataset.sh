...
#SBATCH -J "SYNAPSE_UPDATE"   # job name
#SBATCH -o "/nfs/production/hhe/intact/data/dataset-update-report/update-synapse-dataset-%j.out"   # job output file
...
mvn -U clean install -Pupdate-synapse,${MAVEN_PROFILE} -DdatasetReport=$1 -DselectionReport=$2 -Dmaven.repo.local=repository -Dmaven.test.skip -Ddb=postgres
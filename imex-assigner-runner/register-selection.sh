...
mvn -U clean install -Pregister-selection,${MAVEN_PROFILE} -Dupdate.log.directory=$1 -Dic.username=$2 -Dic.password=$3 -Dic.endpoint=$4 -DinputFile=$6 -Dmaven.repo.local=repository -Dmaven.test.skip -Ddb=oracle